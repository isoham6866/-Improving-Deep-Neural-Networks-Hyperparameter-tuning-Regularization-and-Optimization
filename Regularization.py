"""
Created on Sun August 03 21:26:34 2018
@author: soham ghosh
"""
...
def model(X, Y, learning_rate = 0.3, num_iterations = 60000, print_cost = True, lambd = 0, keep_prob = 1):
    """
...
def compute_cost_with_regularization(A3, Y, parameters, lambd):
    ...
    L2_regularization_cost = np.multiply(np.divide(lambd,(2*m)),np.sum(np.square(W1))+np.sum(np.square(W2))+np.sum(np.square(W3)))
    ### END CODER HERE ###
...
def backward_propagation_with_regularization(X, Y, cache, lambd):
    ...
    ### START CODE HERE ### (approx. 1 line)
    dW3 = 1./m * np.dot(dZ3, A2.T) + np.multiply(np.divide(lambd,m),W3)
    ### END CODE HERE ###
    ...
    ### START CODE HERE ### (approx. 1 line)
    dW2 = 1./m * np.dot(dZ2, A1.T) + np.multiply(np.divide(lambd,m),W2)
    ### END CODE HERE ###
    ...
    ### START CODE HERE ### (approx. 1 line)
    dW1 = 1./m * np.dot(dZ1, X.T) + np.multiply(np.divide(lambd,m),W1)
    ### END CODE HERE ###
